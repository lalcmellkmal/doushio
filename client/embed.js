...
const youtube_url_re = /(?:>>>*?)?(?:https?:\/\/)?(?:www\.|m\.|music\.)?(?:youtu\.be\/|youtube\.com\/watch\/?\?((?:[^\s#&=]+=[^\s#&]*&)*)?v=)([\w-]{11})((?:&[^\s#&=]+=[^\s#&]*)*)&?([#?]t=[\dhms]{1,12})?(?:[&?]si=[\w-]{1,20})?/;
const youtube_time_re = /^[#\?]t=(?:(\d\d?)h)?(?:(\d{1,3})m)?(?:(\d{1,5})s?)?$/;
...
window.twitter_url_re = /(?:>>>*?)?(?:https?:\/\/)?(?:www\.|mobile\.|m\.)?(?:twitter|vxtwitter|x)\.com\/(\w{1,15})\/status\/(\d{4,20})\/?(?:\?[st]=[\w-]+)?&?(?:[st]=[\w-]+)?/;
